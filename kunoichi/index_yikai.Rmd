...
source(file = "~/Data_product_pipeline/kunoichi/xaringan_reveal_parenthetical.R")
library(magick)
...
### This powerpoint aims to use .orange[visualizations] to showcase the process of data analysis from data cleaning to model interpretation

### Specific details within the process are stripped if there is no good way for visualization such as missing value imputation and model ensembling.
...
### This step involves fitting different models and interpret the .yellow[feature importance]
...
## We could find variables with a correlation of at least 0.5 with our dependent variable .cyan[SalePrice]
...
```{r child='flip_plot_seperate_script.Rmd', warning=F, message=F}
```
...
## We could find them via a .cyan[Random Forest] feature importance test.

---

```{r child='lollipop_plot.Rmd', warning=F, message=F}
```
...
data_circular = readRDS("~/Data_product_pipeline/kunoichi/data/data_circular.RDS")
...
### Most missingness indicate the .orange[absence] of that feature (e.g. a pool or fense), which we could impurte as a .pink[None] type 
### some are data misentry and we need to deal with them individually
...
```{r child='coeff_plot.Rmd', warning=F, message=F}
```


---
```{r child='part2_modeling.Rmd', warning=F, message=F}
```
