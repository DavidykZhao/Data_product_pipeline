...
```{r}
train <- read.csv("data/train_house.csv", stringsAsFactors = F)
test <- read.csv("data/test_house.csv", stringsAsFactors = F)
```
...
raincloud_theme <- theme(
  ...
  axis.line.x = element_line(colour = "grey", size = 2, linetype = "solid"),
  axis.line.y = element_line(colour = "grey", size = 2, linetype = "solid"))
...
g <- 
  ...
  theme_bw()+
  theme(plot.subtitle = element_text(color = 'grey12'))+
...
ggplot(all[!is.na(all$SalePrice),], aes(x=MiscFeature, y=SalePrice)) +
        geom_bar(stat='summary', fun.y = "median", aes(fill= MiscFeature)) +
        scale_y_continuous(breaks= seq(0, 800000, by=100000), labels = comma) +
        geom_label(stat = "count", aes(label = ..count.., y = ..count..))+
        scale_fill_manual(values = custom.col[3:length(custom.col)])
...
table(all[!is.na(all$SalePrice),]$Alley)
```
...
all$MasVnrType[2611] <- names(sort(table(all$MasVnrType), decreasing = T))[2] #taking the 2nd value as the 1st is 'none'
...
ggplot(data =all[!is.na(all$SaleType), ], aes(x = PavedDrive, y= SalePrice))+
...
```{r}
set.seed(2018)
quick_RF <- randomForest(x=all[1:1460,-79], y=all$SalePrice[1:1460], ntree=200,importance=TRUE)
...
kable(head(all[all$LowQualFinSF>0, c('GrLivArea', 'X1stFlrSF', 'X2ndFlrSF', 'LowQualFinSF')]))
```
...
saveRDS(train1, file = "training_x.rds")
saveRDS(test1, file = "testing_x.rds")
train1 = readRDS("training_x.rds")
test1 = readRDS("testing_x.rds")


train_df <- cbind(combined[!is.na(all$SalePrice),], 'SalePrice' = all$SalePrice[!is.na(all$SalePrice)])
saveRDS(train_df, file = "training_y_included.rds")

training_y_included = readRDS("training_y_included.rds")
...
```{r}
#autoxgbparset
# change the column name of train_df
# new_col_name = gsub(" ", "_", colnames(train_df))
# colnames(train_df) = new_col_name
# colnames(train_df)
colnames(train_df) = make.names(colnames(train_df))
reg_task <- makeRegrTask(data = train_df, target = "SalePrice")
#system.time(reg_auto <- autoxgboost(reg_task))
#reg_auto
#saveRDS(reg_auto, file = "autoxgboost_160.rds")
```
...
head(sub_avg)
write.csv(sub_avg, file = 'average1.csv', row.names = F)
```
...
shap.plot.dependence(data_long = shap_long, x = 'TotalSqFeet', y = 'Age', color_feature = 'OverallQual') + ggtitle("(A) SHAP values of TotalSqFeet vs. TotalSqFeet")
```
...
# construct a smaller model object from top 20 feature field
...
#shap_int <- shap.prep.interaction(xgb_mod = mod_20, X_train = as.matrix(top_20_feature_field))
#saveRDS(shap_int, "shap_interaction_plot_data.rds")
